...
Four, all registered as free functions. The first three run on queue `email`
and are enqueued by `mboss-nodejs-api` through `DBOSClient`; their names are a
wire contract with that repo — nothing enforces the match but the tests. The
fourth this worker enqueues for itself, on its own queue.
...
| `bounceScan`        | `{ sends: [{ email, operationId }] }`           | Polls each accepted send an hour later and again two days later, and posts the ones that bounced to the API.      |

The scan is on queue `email-status`, uncapped, because it spends nearly all
of its life asleep: this provider offers no bounce webhook, so a poll is the
only way a bounce is ever heard about, and a scan waiting two days must never
...
| `BOUNCE_SCAN_DELAYS_S`     | no       | Defaults to `3600,172800`. When the bounce scan looks, as increasing seconds after the sender enqueued it. |
